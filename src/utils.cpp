...
// error. adapted from CarpetIOHDF5/src/CarpetIOHDF5.hpp.
...
static void MultipoleX_OutputArray(CCTK_ARGUMENTS, FILE *f,
...
void MultipoleX::OutputArrayToFile(CCTK_ARGUMENTS, const std::string &name,
  ...
  if (FILE *fp = MultipoleX_OpenOutputFile(CCTK_PASS_CTOC, name)) {
    MultipoleX_OutputArray(CCTK_PASS_CTOC, fp, th, ph, xs, ys, zs, data);
    fclose(fp);
  }
}
...
void MultipoleX::Output1D(CCTK_ARGUMENTS, const std::string &name,
  ...
  if (FILE *f = MultipoleX_OpenOutputFile(CCTK_PASS_CTOC, name)) {
    fprintf(f, "\"Time = %.19g\n", cctk_time);
...
void MultipoleX::OutputComplexToH5File(
    CCTK_ARGUMENTS, const std::vector<MultipoleX::variable_desc> &vars,
    const CCTK_REAL radii[], const MultipoleX::mode_array &modes) {
...
void MultipoleX::CoordSetup(std::vector<CCTK_REAL> &xhat,
                            std::vector<CCTK_REAL> &yhat,
                            std::vector<CCTK_REAL> &zhat,
                            std::vector<CCTK_REAL> &th,
...
void MultipoleX::ScaleCartesian(const CCTK_REAL r,
                                std::vector<CCTK_REAL> const &xhat,
...
void MultipoleX::Integrate(std::vector<CCTK_REAL> const &array1r,
                           std::vector<CCTK_REAL> const &array1i,
                           std::vector<CCTK_REAL> const &array2r,
                           std::vector<CCTK_REAL> const &array2i,
                           std::vector<CCTK_REAL> const &th,
                           std::vector<CCTK_REAL> const &ph, CCTK_REAL *outre,
                           CCTK_REAL *outim) {
  DECLARE_CCTK_PARAMETERS
  ...
  static std::vector<CCTK_REAL> fr(array_size);
  static std::vector<CCTK_REAL> fi(array_size);
  ...
  if (CCTK_Equals(integration_method, "midpoint")) {
    *outre = MultipoleX::Midpoint2DIntegral(fr.data(), ntheta, nphi, dth, dph);
    *outim = MultipoleX::Midpoint2DIntegral(fi.data(), ntheta, nphi, dth, dph);
  } else if (CCTK_Equals(integration_method, "trapezoidal")) {
    *outre =
        MultipoleX::Trapezoidal2DIntegral(fr.data(), ntheta, nphi, dth, dph);
    *outim =
        MultipoleX::Trapezoidal2DIntegral(fi.data(), ntheta, nphi, dth, dph);
    ...
    *outre =
        MultipoleX::DriscollHealy2DIntegral(fr.data(), ntheta, nphi, dth, dph);
    *outim =
        MultipoleX::DriscollHealy2DIntegral(fi.data(), ntheta, nphi, dth, dph);
  } else {
